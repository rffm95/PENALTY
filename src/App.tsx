...
export default function App() {
  ...
  const shoot = () => {
    ...
    if (gameState === GameState.POWERING) {
      ...
      // 80-90% → esquerda (golo), 90-100% → direita (golo), resto → centro (defesa)
  ...
  // Posição da bola durante/após remate
  const kickSide = result?.side ?? (isKicking ? 'center' : null);
  const ballLeft   = isKicking || isResult
    ? (kickSide === 'left' ? '26%' : kickSide === 'right' ? '68%' : '48%')
    : '26%';
  const ballBottom = isKicking || isResult ? '48%' : '18%';

  // Keeper mergulha para o lado oposto ao golo (se for ao centro, fica no meio e defende)
  const keeperX = isKicking || isResult
    ? (kickSide === 'left' ? 60 : kickSide === 'right' ? -60 : 0)
    : 0;
  const keeperRotate = isKicking || isResult
    ? (kickSide === 'left' ? 20 : kickSide === 'right' ? -20 : 0)
    : 0;

  const powerColor = power >= 80 ? '#22c55e' : power >= 60 ? '#f59e0b' : '#ef4444';
    ...
    >
      ...
      {isPower && (
        <div style={{
          ...
          {/* marcador zona de golo */}
          <div style={{
            position:'absolute', bottom:'80%', left:0, right:0,
            height:1, background:'rgba(255,255,255,.5)'
          }} />
        </div>
      )}
      ...
      {/* GUARDA-REDES (azul) — CENTRO da baliza */}
      <div style={{
        position:'absolute',
        // centrado horizontalmente dentro da baliza
        left:'calc(50% - 35px)',
        bottom:'37%',
        width:70, height:160, transformOrigin:'50% 100%',
        transform:`translateX(${keeperX}px) rotate(${keeperRotate}deg)`,
        transition: isKicking ? 'transform .32s ease-out' : 'none',
        zIndex: 8
      }}>
        <div style={{ position:'absolute', left:'50%', top:0, width:28, height:28, borderRadius:'50%', background:'#f0c090', transform:'translateX(-50%)' }} />
      ...
      {isIdle && (
        <div style={{
          ...
          <div style={{
            background:'rgba(0,0,0,.72)', border:'2px solid rgba(255,255,255,.12)',
        ...
        <div style={{
          ...
          <div style={{
            ...
            {result.success && result.prize && (
              <div style={{ marginTop:12, fontSize:28, fontWeight:800, color:'#FFD700' }}>
                🏆 {result.prize}
              </div>
            )}
